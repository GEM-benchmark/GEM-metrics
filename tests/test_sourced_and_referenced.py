...
class TestSourcedAndReferencedMetric(object):
    ...
    def test_metric(self):
        calculated_metrics = self.get_calculated_metrics(
            ...
            predictions=TestData.predictions,
        )
    ...
    def test_metric_identical_pred_ref(self):
        ...
        calculated_metrics = self.get_calculated_metrics(
            sources=TestData.sources,
            references=TestData.references,
            predictions=TestData.identical_predictions,
        )
        assertDeepAlmostEqual(
            self,
            calculated_metrics,
    ...
    def test_metric_mismatched_empty_tgt(self):
        ...
        calculated_metrics = self.get_calculated_metrics(
            sources=TestData.sources,
            references=TestData.references,
            predictions=TestData.empty_predictions,
        )
        assertDeepAlmostEqual(
            self,
            calculated_metrics,
    ...
    def get_calculated_metrics(
        self, sources: Sources, references: References, predictions: Predictions
    ):
        calculated_metrics = self.metric.compute(
            sources=sources, references=references, predictions=predictions
        )
